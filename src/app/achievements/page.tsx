...
export default function AchievementsPage() {
    ...
    useEffect(() => {
        const fetchItems = async () => {
            try {
                const res = await fetch('/api/items?type=achievement');
    ...
    const handleSave = async (data: Partial<PortfolioItem>) => {
        ...
        try {
            const res = await fetch(url, {
                ...
                body: JSON.stringify({ ...data, type: 'achievement' })
            });

            if (res.ok) {
                const refreshedRes = await fetch('/api/items?type=achievement');
    ...
    return (
        <main className="container section">
            <header style={{ marginBottom: '64px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                <div>
                    <h1 className="section-title" style={{ marginBottom: '16px' }}>Terminal Honors</h1>
                    <p style={{ color: 'var(--text-secondary)', maxWidth: '600px' }}>
                        Verified recognition, certifications, and high-impact milestones recorded within the system.
                    </p>
                </div>
                {isAdmin && (
                    <button
                    ...
                    >
                        <span style={{ fontSize: '20px', lineHeight: 1 }}>+</span> Add Honor
                    </button>
                )}
            </header>
            ...
            {loading ? (
                ...
                <div className="glass-panel" style={{ padding: '100px', textAlign: 'center' }}>
                    ...
                    <h3 style={{ fontSize: '18px', marginBottom: '8px' }}>No Honors Detected</h3>
                    <p style={{ color: 'var(--text-secondary)' }}>The system has not logged any specific achievements yet.</p>
                </div>
            ) : (
